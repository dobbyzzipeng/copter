...
// 	uint16_t buf_4=0,buf_5=0,buf_6=0;
	...
	length=usart_flag&0x03ff;//获取有效数据长度
	for(i=0;i<length;i++)
	{
		buf[i]=ASII_To_Char(data_buf+i);
	}
	
	if((usart_flag&0x0800)==0)//如果是PID参数
	{
		buf_1=buf[1]*100+buf[2]*10+buf[3]+buf[5]/(float)10.0+buf[6]/(float)100.0;//帧格式：AB口100.00口200.00口300.00E
		...
		if((usart_flag&(uint16_t)0x4000)!=0x00)//功能字，PITCH
		{
...
//  		printf("%7.3f\t%7.3f\t%7.3f\r\n",Pitchpid.kp,Pitchpid.ki,Pitchpid.kd);
		...
		else if((usart_flag&(uint16_t)0x2000)!=0x00)//功能字，ROLL
		{
			
			LED1_ON();
// 			printf("%7.3f\t%7.3f\t%7.3f\r\n",Rollpid.kp,Rollpid.ki,Rollpid.kd);
		}
		else if((usart_flag&(uint16_t)0x1000)!=0x00)//功能字，YAW
		{
...
//  		printf("%7.3f\t%7.3f\t%7.3f\r\n",Yawpid.kp,Yawpid.ki,Yawpid.kd);
	...
	{
		buf_4=buf[1]*10000+buf[2]*1000+buf[3]*100+buf[4]*10+buf[5];//帧格式：AB口100.00口200.00口300.00E
